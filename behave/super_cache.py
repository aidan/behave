...
def super_cache_parser(features):
    ...
    for index, runs in step_counts.items():
        if len(runs) > 1:
            ...
            cache_step = Step("", "", "When", "When", "the system state is cached to %s" % index)
            first_scenario, step_index = runs.pop(0)

            first_scenario.insert(step_index, cache_step)

            # Replace the tree in subsequent scenarios up to this point with a cache restore
            restore_step = Step("", "", "When", "When", "the system state is restored from cache %s" % index)
            for other_scenario in runs:
                other_scenario.steps = [restore_step] + other_scenario.steps[step_index:]
